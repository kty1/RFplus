...
def testCases():
    ...
    # Optimal Solution: Either pair (e, f) with H or pair nothing
    '''
    t1 = Tree('(((a,b),c), ((e, f), g));')
    t2 = Tree('(((a,c),b), (g, H));')
    y = compareTree(t1, t2)
    ...
    # Optimal Solution: Pair {c, d} with {i, j} in some combination
    '''
    t1 = Tree('(((((((a,b),c),d),e),f),g),h);')
    t2 = Tree('(((((((e,a),b),h),i),j),f),g);')
    print("Original Trees:\n\n", t1, "\n\n", t2, "\n\n")
    #print(t1, t2)
    y = compareTree(t1, t2)
    #print("swapped:\t", y.swapped, y.init_swap)
    z = y.R_RF_Plus()
    #print("swapped:\t", y.swapped, y.init_swap)
    ...
    # Optimal Solution: Pair {(ca, cb), d} with {i, j} in some combination
    '''
    t1 = Tree('(((((((a,b),(ca, cb)),d),e),f),g),h);')
    t2 = Tree('(((((((e,a),b),h),i),j),f),g);')
    #print(t1, t2)
    y = compareTree(t1, t2)
    #print("swapped:\t", y.swapped, y.init_swap)
    ...
    #print("swapped:\t", y.swapped, y.init_swap)
    print("EF-RF:\n\n", z[0], "\n\n", z[1], "\n\n")
    y2 = compareTree(t1, t2)
    x = y2.R_RF_Plus()
    print("RF:\n\n", x[0], "\n\n", x[1])
    #print(y.cost[y.t.name, 0, 0])
    '''

    # Optimal Solution: Pair {d, g} with {j, k}
    '''
    t1 = Tree('((((((((a, b), c), d), e), f), g), h), i);')
    t2 = Tree('((((a, e), f), j), (b, (k, (h, (c, i)))));')
    #print(t1, t2)
    y = compareTree(t1, t2)
    z = y.R_RF_Plus()
    print(z[0], z[1])
    ...
    # Optimal Solution: Pair {d, e} with {h, i} in some combonation
    '''
    t1 = Tree('(((a, b), c), (d, (e, (f, g))));')
    t2 = Tree('(((((a, g), h), f), i), (j, b));')
    print("Original Trees:\n\n", t1, "\n\n", t2, "\n\n")
    y1 = compareTree(t1, t2)
    x = y1.EF_R_RF()
    print("EF-RF Completions:\n\n", x[0], "\n\n", x[1], "\n\n")
    y2 = compareTree(t1, t2)
    z = y2.R_RF_Plus()
    print("RF Completions:\n\n", z[0], "\n\n", z[1])
    '''

    # Optimal Solution: Pair f and c
    '''
    t1 = Tree('((((a, b), c), d), e);')
    t2 = Tree('((((a, d), f), b), e);')
    y = compareTree(t1, t2)
    ...
    # Optimal Solution: Pair nothing
    '''
    t1 = Tree('((((((a, b), ((c1, c2), c3)), d), e), f), (g1, g2));')
    t2 = Tree('(((((a, d), b), e), (h1, (h2, h3))), f);')
    print("Original trees:\n\n", t1, "\n\n", t2, "\n\n")
    y = compareTree(t1, t2)
    x = y.EF_R_RF()
    print("EF-RF(+):\n\n", x[0], "\n\n", x[1], "\n\n")
    ...
    # Optimal Solution: Pair nothing
    '''
    t1 = Tree('(((((a, b), ((c1, c2), c3)), d), e), f);')
    t2 = Tree('(((((a, d), b), e), ((h1, h4), (h2, h3))), f);')
    print("Original trees:\n\n", t1, "\n\n", t2, "\n\n")
    y = compareTree(t1, t2)
    x = y.EF_R_RF()
    print("EF-RF(+):\n\n", x[0], "\n\n", x[1], "\n\n")
    ...
    # Optimal Solution: Pair two of {k, ((l1, l2), l2)} and {(c1, (c2, c3)), (d1, d2), ((e1, e2), (e3, e4))}
    '''
    t1 = Tree('(((((((((a,b),(c1, (c2, c3))),(d1, d2)),((e1, e2), ((e3, e4), e5))),f),g),h),i),j);')
    t2 = Tree('(((((((((f,a),b),h),k),(((l1, l2), l3), l4)),g),i),m),j);')
    print("Original Trees:\n\n", t1, "\n\n", t2, "\n\n")
    y2 = compareTree(t1, t2)
    x = y2.EF_R_RF()
    print("EF Completions:\n\n", x[0], "\n\n", x[1], "\n\n")
    #print(t1, t2)
    y = compareTree(t1, t2)
    #print("swapped:\t", y.swapped, y.init_swap)
    z = y.R_RF_Plus()
    ...
    # Optimal Solution: Pair p with e and s with k
    '''
    t1 = Tree('(((((((a, b), c), d), e), f), g), (h, (i, (j, (k, (l, (m, n)))))));')
    t2 = Tree('(((((((a, c), o), d), p), f), b), ((q, (i, l)), (r, (m, (s, (n, j))))));')
    print("input:\n\n", t1, "\n\n", t2, "\n\n")
    y = compareTree(t1, t2)
    z = y.R_RF_Plus()
    x = y.EF_R_RF()
    print("EF:\n\n", x[0], "\n\n", x[1], "\n\n")
    print("Opt:\n\n", z[0], "\n\n", z[1])
    print(x[0].robinson_foulds(x[1])[0])
    print(z[0].robinson_foulds(z[1])[0])
    print(y.cost[y.t.name, 0, 0])
    '''


    # Unrooted examples here
    '''
    t1 = Tree('(a, b, (c, d));')
    t2 = Tree('(a, (c, (d, e)));')
    print("input:\n\n", t1, "\n\n", t2, "\n\n")
    ...
    print("EF:\n\n", x[0], "\n\n", x[1], "\n\n")
    print("Opt:\n\n", z[0], "\n\n", z[1])
    print(x[0].robinson_foulds(x[1], unrooted_trees=True)[0])
    print(z[0].robinson_foulds(z[1], unrooted_trees=True)[0])
    print(y.cost[y.t.name, 0, 0])
    ...
    # Everything below this will test pairs of trees produced from sagephy-1.0.0
    # The file sagephyHandler.py is only used to read some precomputed trees from sagephy
    # getLargeSagephyTrees() will return 5 trees with 900 - 1000 leaves
    # getMedSagephyTrees() will return 5 trees with 200 - 300 leaves
    # getSmallSagephyTrees() will return 5 trees with 20 - 30 leaves

    #trees = sagephyHandler.getLargeSagephyTrees()
    '''
    t1, t2 = trees[2], trees[4]
    y = compareTree(t1, t2)
    x = y.EF_R_RF()
    EF = x[0].robinson_foulds(x[1])[0]
    print("EF Size:\t", len(x[0]))
    #print(y.t)
    ...
    #print(y.redLeaves, "\n\n", y.yellowLeaves)
    z = y.R_RF_Plus()
    print("Expected RF+ DIST:\t", EF + 2 * y.cost[y.orig_root_name, 0, 0])
    print(y.cost[y.orig_root_name, 0, 0], y.cost[y.orig_root_name, 0, 1])
    #print("Opt:\n\n", z[0], "\n\n", z[1], "\n\n")
    #print(y.redLeaves, "\n\n", y.yellowLeaves)
    print("RF Size:\t", len(z[0]))
    print("RF+ DIST:\t", z[0].robinson_foulds(z[1])[0])
    print("\n\n")
    print(y.t)
    '''

    # Check if ext clades were merged properly
    '''
    for node in y.t.traverse("postorder"):

        if node.name != '':
            print("NODE:\t", node.name)
            print("COLOR:\t", y.tcolors[node.name])
            if node.up != None:
                print("PARENT:\t", node.up.name)
            print("CHILDREN:\t", node.children)
            if len(node.children) > 0:
                if node.children[0].name != '':
                    print("LEFTMAX:\t", y.cMax1[0, node.children[0].name], y.cMax1[1, node.children[0].name])
                if node.children[1].name != '':
                    print("RIGHTMAX:\t", y.cMax1[0, node.children[1].name], y.cMax1[1, node.children[1].name])
            print("OPTN:\t", y.optN1[node.name])
            print("OPTc:\t", y.optc1[node.name])
            print("OPTCOST:\t", y.cost[node.name, y.optN1[node.name], y.optc1[node.name]])
            print(y.order1[0, node.name], "\n", y.order1[1, node.name], "\n\n")
    '''
    ...
    for i in range(len(trees)):
        for j in range(i):
            ...
            #print("Opt:\n\n", z[0], "\n\n", z[1], "\n\n")
            #print(y.redLeaves, "\n\n", y.yellowLeaves)
            print("Check Paired RF+ DIST:\t", z[0].robinson_foulds(z[1])[0])
            print("\n\n")
    '''
    ...
    '''
            for node in y.t.traverse("postorder"):
                if node.name == 'H80':
                    print(y.tcolors[node.name])
                if node.name != '' and y.optN1[node.name] > 0:
                    print("NODE:\t", node.name)
                    if node.up != None:
                        print("PARENT:\t", node.up.name)
                    print("CHILDREN:\t", node.children)
    ...
    '''
            ...
            # Check if optimal values were computed correctly
    '''
            for node in y.t.traverse("postorder"):
                if node.name != '':
                    ...
                    if node.up != None:
                        print("Parent:\t", node.up.name)
                    print("Opt N:\t", y.optN1[node.name])
                    print("Opt C:\t", y.optc1[node.name])
                    print("CMax:\t", y.cMax1[y.optc1[node.name], node.name], "\n\n")
    '''
...
def main():
    ...
    parser.add_argument("-o", "--outputfile", type = str, help = "The output file to which the rfdistance or trees in newick format will be printed")
    ...
    if args.analysisfile:
        unionsize, intersectsize, rfplus, rfminus, efrf, eftime, rftime, redsubs = {}, {}, {}, {}, {}, {}, {}, {}


    for i in range(len(msn)):
        for j in range(i):
            print("I, J:\t", i, j)
            print("Length of tree {}:\t".format(i), len(msn[i]))
            ...
            if(args.outputfile and args.rfdistance):
                fileW.write("lines {} and {}: The RF(-) distance is {}. The EF-RF(+) distance is {}. The RF(+) distance is {}.\n".format(i+1, j+1, a, z1, z2))
                ...
                if args.extraneousfree:
                    ...
                    #fileW.write("\n--------------------------------------------------------------------------------------------------------------------------\n\n")
            elif(args.rfdistance):
                print("lines {} and {}: The RF(-) distance is {}. The EF-RF(+) distance is {}. The RF(+) distance is {}.\n".format(i+1, j+1, a, z1, z2))
            else:
                if args.extraneousfree:
            ...
            if args.analysisfile:
                unionsize[(j, i)] = len(y2[0])
                intersectsize[(j, i)] = len(y2[0]) - len(y.yellowLeaves) - len(y.redLeaves)
                rfplus[(j, i)] = z2
                efrf[(j, i)] = z1
                rfminus[(j, i)] = a
                eftime[(j, i)] = y.EF_time
                rftime[(j, i)] = y.RF_time
                redsubs[(j, i)] = y.cMax[0, y.t.name] + y.cMax[1, y.t.name]


    if args.analysisfile:
        with open(args.analysisfile, 'w', newline='') as csvfile:
            fieldnames = ['Tree_1', 'Tree_2', 'Tree_1_size', 'Tree_2_size', 'RF(-)', 'EF-RF(+)', 'RF(+)', 'Union_size', 'Intersect_size', 'Red Subtrees', 'EF-RF(+) Time', 'RF(+) Time']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

            writer.writeheader()
            for i in range(len(msn)):
                for j in range(i):
                    writer.writerow({'Tree_1': str(j), 'Tree_2': str(i), 'Tree_1_size': str(len(msn[j])), 'Tree_2_size': str(len(msn[i])), 'RF(-)': str(rfminus[(j, i)]), 'EF-RF(+)': str(efrf[(j, i)]), 'RF(+)': str(rfplus[(j, i)]), 'Union_size': str(unionsize[(j, i)]), 'Intersect_size': str(intersectsize[(j, i)]), 'Red Subtrees': str(redsubs[(j, i)]), 'EF-RF(+) Time': str(eftime[(j, i)]), 'RF(+) Time': str(rftime[(j, i)])})
